...
internal class TestTypeLookup : Test {
	...
	Void testExactMacth() {
		...
		verifyErrMsg(ArgNotFoundErr#, "Could not find match for Type afBeanUtils::TestTypeLookup.") |t| {
			try {
				ap.findExact(TestTypeLookup#)
			} catch (ArgNotFoundErr nfe) {
		...
		verifyErrMsg(ArgNotFoundErr#, "Could not find match for Type afBeanUtils::T_InnerArgErr.") |t| {
			try {
